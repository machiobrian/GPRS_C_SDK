...
void EventDispatch(API_Event_t* pEvent) {
	...
	case API_EVENT_ID_NO_SIMCARD:
		Trace(10, "NO SIMCARD%d", pEvent->param1);
		break;
	...
	case API_EVENT_ID_NETWORK_REGISTERED_ROAMING:
	{
		...
		if (!ret)
			Trace(1, "get attach status fail");
	...
	case API_EVENT_ID_NETWORK_CELL_INFO: {
		...
		Trace(2, "network cell information, serving cell number:1, neighbour cell number:%d", number - 1);

		for (int i = 0; i < number; ++i) {
			Trace(2, "cell %d info: %d%d%d,%d%d%d,%d,%d,%d,%d,%d,%d ", i,
				location[i].sMcc[0], location[i].sMcc[1], location[1].sMcc[2],
				location[i].sMnc[0], location[i].sMnc[1], location[1].sMnc[2],
...
void MainTask(void* pData) {
	...
	secondTaskHandle = OS_CreateTask(
		secondTask,
		NULL,
		NULL,
		SECOND_TASK_STACK_SIZE,
		SECOND_TASK_PRIORITY,
		0,
		0,
		SECOND_TASK_NAME
	);
